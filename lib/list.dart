...
class _MyHomePage extends State<MyHomePage> {
  ...
  Widget build(BuildContext context) {
    List ages = clients
        .map((e) => e.age!)
        .toList(); //Enviar todas las edades a una lista
    List<int> intList = ages
        .map((s) => int.parse(s))
        .toList(); //Enviar todas las edades a una lista de enteros
    double average = intList.isEmpty
        ? 0.0
        : intList.reduce((value, element) => value + element) /
            intList.length; //Calcular promedio edad
    var averageRounded = num.parse(average.toStringAsFixed(2));
    final List hobbiesList = clients
        .map((e) => e.hobbies!)
        .toList(); //Tomar hobbies de lista clientes y enviarlos a una lista única.
    print(hobbiesList);
    print(intList);
    ...
    String result =
        ''; //String para guardar todos los valores después del split
    for (String hobbies in frequencyMap.keys) {
      int count = frequencyMap[hobbies]!;
      String wordCountText =
          "El hobbie $hobbies le gusta a $count persona(s)\n";
                  ...
                  child: Container(
                      ...
                      child: Center(
                          child: Text("Usuarios: ${clients.length}",
                              style:
                                  GoogleFonts.blackOpsOne(fontSize: size02))))),
              Expanded(
                  child: Container(
                      decoration: BoxDecoration(
                        borderRadius: BorderRadius.all(Radius.circular(5.0)),
              ...
              Expanded(
                  child: Container(
                      ...
                      child: Center(
                          child: Text(result,
                              style:
                                  GoogleFonts.blackOpsOne(fontSize: size03))))),
            ])),
              ...
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  return ListTile(
                        ...
                        if (newContact != null) {
                          setState(() {
                            ...
                            messageResponse(
                                context,
                                newContact.name +
                                    " ha sido actualizado ${clients.length}.");
                          });
                        }
                      });
                    },
                    ...
                    title: Text(
                        clients[index].name + " " + clients[index].surname),
...
class Client {
  ...
  Client(
      ...
      this.phone,
      this.age,
      this.hobbies}); //Todos los campos del cliente
}
